...
For now, copy the starter code into the `wall_follower` starter code so roslaunch will work correctly (make sure to save your wall follower code somewhere first!). You can either manuaully copy in the code or use `cp ptFieldSim.py /your/path/wall_follower.py` in your terminal/powershell. Then run `roslaunch wall_follower wall_follower.launch` just like yesterday.
